...
import { FlaskConical, Plus, Search, Filter, Edit2, Trash2, X } from 'lucide-react';
import { supabase, type Fertilizer } from '../supabase';
...
export default function FertilizersInventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from('fertilizers').insert([formData]);
      if (error) throw error;
      setIsModalOpen(false);
      setFormData({ name: '', type: 'Organic', quantity: 0, unit: 'Sacks', batch_number: '' });
      fetchItems();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Fertilizers Inventory</h1>
          <p className="text-slate-500">Track and manage fertilizer stocks.</p>
        </div>
        <button 
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by name, type or batch..."
            value={searchTerm}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Fertilizer Name</th>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        item.type === 'Organic' ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"
                      )}>
                        {item.type}
                      </span>
                    </td>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div 
                            className={cn(
                              "h-full rounded-full",
                              item.quantity > 50 ? "bg-emerald-500" : item.quantity > 10 ? "bg-amber-500" : "bg-rose-500"
                            )}
                    ...
                    <td className="px-6 py-4 text-sm font-mono text-slate-500">{item.batch_number}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-emerald-600 text-white">
                <h3 className="text-xl font-bold">Add Fertilizer</h3>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Fertilizer Name</label>
                    <input
                      required
                      type="text"
                      value={formData.name}
