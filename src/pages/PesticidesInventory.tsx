...
import { ShieldAlert, Plus, Search, Filter, Edit2, Trash2, X } from 'lucide-react';
import { supabase, type Pesticide } from '../supabase';
...
export default function PesticidesInventory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from('pesticides').insert([formData]);
      if (error) throw error;
      setIsModalOpen(false);
      setFormData({ name: '', type: 'Insecticide', quantity: 0, unit: 'Bottles' });
      fetchItems();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Pesticides Inventory</h1>
          <p className="text-slate-500">Manage crop protection chemicals and pesticides.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Pesticide Name</th>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div 
                            className={cn(
                              "h-full rounded-full",
                              item.quantity > 50 ? "bg-emerald-500" : item.quantity > 15 ? "bg-amber-500" : "bg-rose-500"
                            )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-emerald-600 text-white">
                <h3 className="text-xl font-bold">Add Pesticide</h3>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Pesticide Name</label>
                    <input
                      required
                      type="text"
                      value={formData.name}
